...
def get_response_token_ids(input_token_ids, outputs):
    response_token_ids = []
    response_logprobs = []
    second_input_token_ids = []
    for request_output in outputs:  # List[RequestOutput]
        o = request_output.outputs
        for output in o:  # List[CompletionOutput], usually len == 1
            if output.token_ids[-1] == 151649:
                response_token_ids.append(list(output.token_ids))
            else:
                response_token_ids.append(list(output.token_ids) + [151649])
    ...
    for prompt, first_response in zip(input_token_ids, response_token_ids):
        second_input_token_ids.append(list(prompt) + list(first_response))
        
    return second_input_token_ids, response_token_ids, response_logprobs

def get_second_response_token_ids(second_outputs):
    second_response_token_ids = []
    second_response_logprobs = []
    for request_output in second_outputs:  # List[RequestOutput]
        o = request_output.outputs
        for output in o:  # List[CompletionOutput], usually len == 1
            second_response_token_ids.append(output.token_ids)
            logprobs_dicts = output.logprobs
            if logprobs_dicts is not None:
                logprob = []
                for logprobs_dict, id in zip(logprobs_dicts, output.token_ids):
                    logprob.append(logprobs_dict[id].logprob)
                second_response_logprobs.append(logprob)
        
    return second_response_token_ids, second_response_logprobs

def post_process(response_token_ids, response_logprobs, tokenizer):
    output_token_ids = []
    logprobs = []
    for r in response_token_ids:
        output_token_ids.append(torch.tensor(r))
    if len(response_logprobs) > 0:
        for l in response_logprobs:
            logprobs.append(torch.tensor(l))
...
def _post_process_outputs(request_outputs, tokenizer):
    output_token_ids = []
    logprobs = []
